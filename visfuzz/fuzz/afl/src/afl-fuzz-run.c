...
common_fuzz_stuff(afl_state_t *afl, u8 *out_buf, u32 len) {
  ...
  /*   there has be changed   */  //afl->fsrv.map_size
/**********************************************************************************/
  u16 *trace_counts = (u16 *)afl->fsrv.trace_bits;
  for (u32 i = 0; i < MAP_SIZE / sizeof(u32); ++i) /*   there has be changed   */
  {
    afl->block_freq[i] += trace_counts[i];
  }
  if (afl->stage_cur % afl->stats_update_freq)
  return 0;

  char *fname = alloc_printf("%s/block_freq.json", afl->out_dir);
  FILE *f = fopen(fname, "w");
  if (f == NULL) 
    PFATAL("Unable to open '%s'", fname);

  fprintf(f, "{\"freq\":[%llu", (long long unsigned int)afl->block_freq[0]);
  for (u32 i = 1; i < MAP_SIZE / sizeof(u32); ++i)  /*   there has be changed   */
  {
    fprintf(f, ",%llu", (long long unsigned int)afl->block_freq[i]);
  }
  fprintf(f, "]}");
  fclose(f);
  ck_free(fname);
